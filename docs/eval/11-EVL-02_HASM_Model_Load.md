...
| --- | --- | --- |
| `TC-02-REACT-001` to `TC-02-REACT-005`, `TC-02-REACT-PERF-001`, active-lock and missing-storage route cases | `test/seq02.test.jsx` via `npm run test:eval-02` | Frontend: EVAL-02 React and IPC tests |
...
| `TC-02-RUST-PERF-001` to `TC-02-RUST-PERF-003` | `src-tauri/src/hasm/model_commands.rs` via `cargo test model_commands` | Rust: EVAL-02 Rust model loading tests |
...
| **TC-02-E2E-PERF-001** | `REQ-02-RULE-007``REQ-02-FUNC-401` | Positive (Scale) | Open a 60,000-Entity Package | 1. Prepare a package with approximately 15,000 entities of each type.<br/>2. Open it in a release build on local SSD storage and time from `/loading-model` mount to `/visualizer` navigation. | 1. Total elapsed time is under 20,000 ms.<br/>2. Progress never stalls for 10,000 ms, so no watchdog error occurs.<br/>3. Entity counts on the visualizer match the package. |
| **TC-02-E2E-PERF-002** | `REQ-02-RULE-009``REQ-02-FUNC-403` | Positive (Warm Reopen) | Reopen an Unchanged Large Package | 1. Open the 60,000-entity package once and close it.<br/>2. Record the `main.db` modification time and reopen the package. | 1. The second open is faster than the first.<br/>2. No bootstrap `INSERT` is issued; entity row counts are unchanged. |
...
| **TC-02-REACT-PERF-001** | `REQ-02-RULE-008``REQ-02-FUNC-406``REQ-02-FUNC-407` | Positive (Metadata-Only Load) | `LoadingModelPage.tsx` | 1. Mock `load_hasm_model_db` to resolve entities carrying `markdownPath` and an empty `markdown`.<br/>2. Allow verification to resolve. | 1. Navigates to `/visualizer`.<br/>2. `load_hasm_model_db` is invoked exactly once.<br/>3. The metadata-only model is forwarded verbatim to `verify_hasm_storage`. |
...
| **TC-02-RUST-PERF-001** | `REQ-02-RULE-007``REQ-02-RULE-008``REQ-02-FUNC-406``REQ-02-FUNC-408` | Positive (Scale) | `service::read_model_database_with_progress` / `find_unreferenced_entity_folders` | 1. Build a database-less workspace holding 500 folder-only entities of each type.<br/>2. Load it with `include_markdown = false`.<br/>3. Run the unreferenced-folder scan against the expected paths. | 1. All 2,000 entities are returned.<br/>2. Every entity has an empty `markdown` and a populated `markdown_path`.<br/>3. The scan reports no unreferenced folders and completes without a quadratic scan. |
| **TC-02-RUST-PERF-002** | `REQ-02-RULE-009``REQ-02-FUNC-402``REQ-02-FUNC-403` | Positive (Idempotent Bootstrap) | `service::read_model_database_with_progress` | 1. Build a database-less workspace holding 200 folder-only entities of each type.<br/>2. Load it twice. | 1. The second load returns the same entity counts.<br/>2. Each table holds exactly one row per entity folder; no duplicate bootstrap rows are created. |
| **TC-02-RUST-PERF-003** | `REQ-02-FUNC-404``REQ-02-FUNC-405` | Positive (Storage Tuning) | `service::open_connection` / `service::ensure_schema` | 1. Load a small workspace to create `main.db`.<br/>2. Query `PRAGMA journal_mode` and `sqlite_master`.<br/>3. Run `EXPLAIN QUERY PLAN` for the PERSON list ordering. | 1. `journal_mode` is `wal`.<br/>2. `idx_person_sort`, `idx_experience_sort`, `idx_fact_sort`, `idx_link_sort`, and `idx_experience_person` all exist.<br/>3. The plan uses `idx_person_sort` rather than a temporary B-tree sort. |